...
class IOSRxPublishSubject : IOSRxObservable, SharedRxPublishSubject {
    
    public init(publishSubject: PublishSubject<Any>) {
        super.init(observable: publishSubject)
    }
    
    private func asSubject() -> PublishSubject<Any> {
        return observable as! PublishSubject<Any>
    }
